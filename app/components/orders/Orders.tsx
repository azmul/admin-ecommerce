...
export default function Link() {
  ...
  const columns: any = [
    ...
    },
    {
      ...
      // eslint-disable-next-line react/display-name
      render: (_name: string, record: any) => <>
      {record.shippingStatus === DELIVERY.PENDING && <b style={{color: 'blue'}}>Pending</b>}
      {record.shippingStatus === DELIVERY.CHECKING && <b style={{color: 'black'}}>Checking</b>}
      {record.shippingStatus === DELIVERY.PROCESSING && <b style={{color: 'orange'}}>Processing</b>}
      {record.shippingStatus === DELIVERY.SHIPPED && <b style={{color: 'green'}}>Shipped</b>}
      {record.shippingStatus === DELIVERY.DELIVERED && <b style={{color: 'green'}}>Delivered</b>}
      {record.shippingStatus === DELIVERY.CANCELED && <b style={{color: 'red'}}>Canceled</b>}
      </>
    },
    {
      title: "Created At",
      ...
      >
        <Descriptions column={2} title="Price Info">
          ...
          <Descriptions.Item label="Delivery Status">
            {item && item.shippingStatus === DELIVERY.PENDING && <b style={{color: 'blue'}}>Pending</b>}
            {item && item.shippingStatus === DELIVERY.CHECKING && <b style={{color: 'black'}}>Checking</b>}
            {item && item.shippingStatus === DELIVERY.PROCESSING && <b style={{color: 'orange'}}>Processing</b>}
            {item && item.shippingStatus === DELIVERY.SHIPPED && <b style={{color: 'green'}}>Shipped</b>}
            {item && item.shippingStatus === DELIVERY.DELIVERED && <b style={{color: 'green'}}>Delivered</b>}
            {item && item.shippingStatus === DELIVERY.CANCELED && <b style={{color: 'red'}}>Canceled</b>}
          </Descriptions.Item>
          <Descriptions.Item label="Created At">{moment(item?.createdAt).format('MMMM Do YYYY, h:mm:ss a')}</Descriptions.Item>
        </Descriptions>
        ...
        {item && item?.products.map((product: any) => <>{
          <Descriptions column={2} title="Product Info">
            ...
            <Descriptions.Item label="Discounted Price">{product.finalDiscountedPrice}৳</Descriptions.Item>
            <Descriptions.Item label="Offer End">{product.offerEnd}</Descriptions.Item>
            <Descriptions.Item label="Image"><img alt="Product Image" src={product.image[0]} width="100" height="100" /></Descriptions.Item>
          </Descriptions>
        }</>)}
