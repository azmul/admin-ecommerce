...
export default function HomeopathyDoctorDetails(){
    ...
    const getdoctor = async () => {
          try {
            if(typeof(id) === "string") {
              setloading(true);
              const response: any = await homeopathyApi.getHomeopathyDoctor(id);
              setdoctor(response);
            }
          } catch(error: any) {
            message.error(error?.message);
            router.push(`/homeopathy/doctors`)
          } finally {
            setloading(false);
          }
        }
    ...
    const handleActiveStatus =  useCallback (async () => {
      try {
          if(typeof(id) === "string") {
            setloading(true);
            const response: any = await homeopathyApi.activeDoctor(id);
            setdoctor(response);
          }
        } catch(error: any) {
          message.error(error?.message);
  ...
  const handleInActiveStatus =  useCallback (async () => {
      try {
          if(typeof(id) === "string") {
            setloading(true);
            const response: any = await homeopathyApi.deleteDoctor(id);
            setdoctor(response);
          }
        } catch(error: any) {
          message.error(error?.message);
